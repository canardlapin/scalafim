...
For a resizable host, use `OrthogonalLayout.adaptive(marginPixels = 12.0,
gapPixels = 8.0)`. It chooses the greatest fitted image area among row, column
and L arrangements from the physical slice dimensions and current device size.
The insets use logical pixels and remain constant as the viewport resizes; if
the viewport cannot contain them, they shrink proportionally. This optimizes
among the three supported arrangements, not arbitrary rectangle packings.

To enlarge one plane, use `OrthogonalLayout.make(12.0, 8.0,
OrthogonalArrangement.SinglePlane(AnatomicalPlane.Axial),
OrthogonalLayoutUnits.LogicalPixels)`. Both constructors return an `Either`.
Change the session layout, retaining its state, and compile with the new device
size. Physical aspect ratio, cursor, convention and each plane's zoom/pan survive
switching back to the adaptive layout. The focused plane fills the inset
viewport as far as its aspect ratio permits.

Use `frame.visiblePanels` for host overlays and pointer targets, and
`frame.visibleReadouts` for displayed readouts. Named `frame.panels` receipts
remain available for all planes, with explicit `visible` flags. Hidden planes
are neither rendered nor sampled, have empty layer readouts, and cannot receive
pointer picks. Native host event adapters use the visible receipts automatically.
