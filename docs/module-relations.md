...
+-- locus-data
|   +-- image
|   +-- surface       also depends on image, standalone graph4s, zarr4s core
...
+-- surface           also depends on image, locus-data, zarr4s core
...
+-- group             also depends on image, dataset, design, estimates
...
standalone zarr4s
+-- surface           core only: portable SHA-256 for digest-bound surface declarations
...
| `surface` | Meshes, exact topology/order locus domains, vertex fields, region-backed surface ROIs, quotient-backed labels, geodesic searchlights, graph4s interop, cross-platform GIFTI ingestion with coordinate declarations, JVM FreeSurfer readers, and exact-reference volume-to-cortical-mesh route admission with digest-bound frame declarations and affine or displacement point-map frame bridges. | standalone graph4s, `image`, `locus-data`; standalone zarr4s core (portable SHA-256 for digest-bound surface declarations, confined to `SurfaceDigest`); JVM only: ujson for point-map manifests | Atlas metadata, MVPA plans, or whole spatial graph compilation. |
