...
external Python packages, then runs the scenario testkit, HRF, HRF laws,
...
movement below the measured 2026-08-04 baseline while still rejecting a
...
| --- | ---: | ---: | ---: |
| `hrfJVM` | 72.88% | 63.02% | 70% / 60% |
| `designJVM` | 72.84% | 61.16% | 70% / 58% |
| `modelJVM` | 61.53% | 56.00% | 58% / 52% |
| `fitJVM` | 77.28% | 64.69% | 75% / 62% |

## Compiler and source policy

Production and test sources in `hrf`, `design`, `model`, `fit`, and
...
| Generated-law calibration | weekly schedule or manual dispatch | advisory | 300 deterministic cases per property on JVM and Scala.js |
...
| First-level performance | weekly schedule or manual dispatch | required release evidence | raw HRF/fit JMH reports plus a budgeted time/allocation receipt |
...
Mutation testing is deferred while generated algebraic laws, independent R and
Nilearn receipts, and scoped coverage supply the stronger immediate signal.
Pilot mutation testing on OLS rank decisions, censor/reset GLS branches, and
design policy lowering once the current baseline is stable; it becomes a
release decision before a compatibility-bearing release.
...
The court distinguishes basis response, convolution, integration, fit
planning, reusable factorization, multiresponse fitting, and chunk assembly.
