...
out_file <- Sys.getenv(
  "DMS_RECEIPT_OUT",
  file.path("docs", "scenarios", "fixtures", "fit.dms-multiphase-dsl.corrected-spmg.v1.r.json")
)
scala_out <- Sys.getenv(
  "DMS_SCALA_OUT",
  file.path(
    "modules", "fit", "shared", "src", "test", "scala",
    "scalafim", "fmri", "fit", "fixtures", "CorrectedSpmgDmsRFixture.scala"
  )
)
scala_object <- Sys.getenv("DMS_SCALA_OBJECT", "CorrectedSpmgDmsRFixture")
...
source <- list(
  ...
  fmridesign_source_md5 = source_md5(fmridesign_root),
  fmrihrf_source_md5 = source_md5(fmrihrf_root),
...
scala_matrices <- function(values) scala_vector(values, scala_matrix)
...
writeLines(
  c(
    ...
    if (identical(scala_object, "DmsRFixture")) "final case class DmsTExpected(estimate: Double, standardError: Double, statistic: Double)" else NULL,
    if (identical(scala_object, "DmsRFixture")) "final case class DmsFExpected(numeratorDf: Int, denominatorDf: Int, statistic: Double)" else NULL,
    if (identical(scala_object, "DmsRFixture")) "" else NULL,
    ...
    paste0("  val designColumnKeys: Vector[String] = ", scala_strings(colnames(design))),
    paste0("  val designColumnSums: Vector[Double] = ", scala_doubles(colSums(design))),
    paste0("  val designColumnSquaredNorms: Vector[Double] = ", scala_doubles(colSums(design * design))),
    paste0("  val designColumnProbe: Vector[Double] = ", scala_doubles(drop(crossprod(sin(seq_len(nrow(design)) * 0.131) + cos(seq_len(nrow(design)) * 0.071), design)))),
    ...
    paste0("  val runCoefficients: Vector[Vector[Double]] = ", scala_matrix(lapply(run_fits, `[[`, "coefficients"))),
    paste0("  val runCovariance: Vector[Vector[Vector[Double]]] = ", scala_matrices(lapply(run_fits, function(run) matrix_rows(run$covariance)))),
    paste0("  val fixedCoefficients: Vector[Double] = ", scala_doubles(fixed_coefficients)),
    paste0("  val fixedCovariance: Vector[Vector[Double]] = ", scala_matrix(matrix_rows(fixed_covariance))),
