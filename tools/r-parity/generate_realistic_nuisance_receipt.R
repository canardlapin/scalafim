...
out_file <- Sys.getenv(
  ...
  file.path("docs", "scenarios", "fixtures", "fit.realistic-nuisance.corrected-spmg.v1.r.json")
)
scala_out <- Sys.getenv(
  "REALISTIC_NUISANCE_SCALA_OUT",
  file.path(
    "modules", "fit", "shared", "src", "test", "scala", "scalafim",
    "fmri", "fit", "fixtures", "CorrectedSpmgRealisticNuisanceRFixture.scala"
  )
)
scala_object <- Sys.getenv("REALISTIC_NUISANCE_SCALA_OBJECT", "CorrectedSpmgRealisticNuisanceRFixture")
...
source <- list(
  ...
  fmrihrf_source_md5 = {
    paths <- c(file.path(hrf_pkg, "DESCRIPTION"), list.files(file.path(hrf_pkg, "R"), pattern = "[.]R$", full.names = TRUE))
    values <- tools::md5sum(paths)
    names(values) <- substring(paths, nchar(hrf_pkg) + 2L)
    as.list(values)
  },
...
outputs <- list(
  ...
  full_design_column_keys = colnames(full_design),
  full_design = matrix_rows(full_design),
  response = response,
...
scala_matrix <- function(value) scala_vector(value, scala_doubles)
...
writeLines(
  c(
    ...
    if (identical(scala_object, "RealisticNuisanceRFixture")) "final case class RealisticNuisanceRun(columns: Vector[(String, Vector[Double])])" else NULL,
    if (identical(scala_object, "RealisticNuisanceRFixture")) "final case class RealisticNuisanceTExpected(estimate: Double, standardError: Double, statistic: Double)" else NULL,
    if (identical(scala_object, "RealisticNuisanceRFixture")) "final case class RealisticNuisanceFExpected(numeratorDf: Int, denominatorDf: Int, statistic: Double)" else NULL,
    if (identical(scala_object, "RealisticNuisanceRFixture")) "" else NULL,
    paste0("object ", scala_object, ":"),
    ...
    paste0("  val taskDesign: Vector[Vector[Double]] = ", scala_matrix(matrix_rows(task_design))),
    paste0("  val fullDesignColumnKeys: Vector[String] = ", scala_strings(colnames(full_design))),
    paste0("  val fullDesignColumnSums: Vector[Double] = ", scala_doubles(colSums(full_design))),
    paste0("  val fullDesignColumnSquaredNorms: Vector[Double] = ", scala_doubles(colSums(full_design * full_design))),
    paste0("  val fullDesignColumnProbe: Vector[Double] = ", scala_doubles(drop(crossprod(sin(seq_len(nrow(full_design)) * 0.131) + cos(seq_len(nrow(full_design)) * 0.071), full_design)))),
