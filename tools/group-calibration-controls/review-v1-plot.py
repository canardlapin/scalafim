...
a.set_xlabel('False positives (%)');a.set_title('A  Error shape changes the conclusion',loc='left',size=12)
...
for m,label,col in [('native_PM_mKH','Native PM/mKH',orange),('plugin_GLS_t','Plug-in WLS t',purple),('plugin_HC3_Satt','WLS HC3 / Satterthwaite',blue),('oracle_WLS_t','True-variance WLS t',green)]:
    ...
    b.errorbar(range(3),v,yerr=np.array([v-ci[:,0],ci[:,1]-v]),fmt='o-',color=col,label=label,ms=5,capsize=2,lw=1.5)
...
b.set_ylabel('False positives (%)');b.set_xlabel('First-level variance information');b.set_title('B  Noisy first-level SEs inflate PM/mKH',loc='left',size=12)
...
fig.text(.035,.955,'Feasible precision weighting: good Gaussian calibration, visible power costs',size=17,weight='bold')
...
for row,n in enumerate([20,80]):
    for i,(v,t,df) in enumerate(cases):
        ...
        for m,col,marker,off in [('fixed_vhat_HC3_Satt',blue,'o',-.13),('unweighted_HC3_Satt',orange,'s',.13)]:
            ...
            axs[row,1].plot(100*r['methods'][m]['powerAtNominalAlpha'],i+off,marker,color=col,ms=4.5)
    for col in [0,1]:
...
fig.text(.035,.050,'24 Gaussian settings, 10,000 studies each. Left bars: pointwise 95% binomial intervals. Power uses the implemented nominal 5% threshold.',size=9,color=gray)
