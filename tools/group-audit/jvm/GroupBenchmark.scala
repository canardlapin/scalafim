...
object GroupBenchmark:
  ...
  def main(args:Array[String]):Unit =
    ...
    val y=matrix(n,m)((i,s)=>0.5+0.4*x(i,1)+math.sin((i+1)*(s%37+1)*0.13))
    val vs=matrix(n,m)((i,s)=>0.1+0.03*(i%7)+0.01*(s%11))
    ...
    while r<3 do
      ...
      println(s"BENCH,$mode,$n,$p,$m,$r,$setup,$fitTime,$fitAlloc,$contrastTime,$fdrTime,$gc,$consumed,$threadCpu,$processCpu")
      r+=1
