...
/** Native framebuffer gate: nearest regions must retain palette colors away from boundaries. */
object JavaFxSurfaceNearestProbe:
  def main(args: Array[String]): Unit =
    ...
    Platform.startup: () =>
      ...
      try
        ...
        for
          ...
          weights = Vector(pick.barycentricA, pick.barycentricB, pick.barycentricC).sorted.reverse
          if weights.last > 0.04 && weights(0) - weights(1) > 0.04
        do
          ...
          checked += 1
        require(checked > 5000, s"insufficient interior coverage: $checked")
        require(maximumError <= 1, s"nearest colors bled: maximum channel error $maximumError")
        ...
        try
          ...
          val picked = controller.pick(new _root_.javafx.scene.input.PickResult(chunk.view,
            new _root_.javafx.geometry.Point3D(-0.7, 0.8, 0.0), 4.0, 11, _root_.javafx.geometry.Point2D.ZERO)).toOption.get
          require(picked.face.index == 1 && picked.vertex.index == 3,
            s"render-corner pick lost source identity: $picked")
          require(math.abs(picked.barycentricA - 0.1) < 1e-6 && math.abs(picked.barycentricC - 0.75) < 1e-6)
        ...
        javax.imageio.ImageIO.write(out, "png", new java.io.File("/private/tmp/scalafim-nearest-javafx.png"))
        println(s"nearest_javafx=pass checked=$checked maximum_channel_error=$maximumError source_vertices=4 render_vertices=36 render_faces=12")
    ...
    Platform.exit()
    require(completed, "JavaFX nearest probe timed out")
