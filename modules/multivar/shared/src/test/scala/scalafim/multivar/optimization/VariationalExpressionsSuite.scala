...
class VariationalExpressionsSuite extends munit.FunSuite:
  ...
  test("only linear maps expose an algebraic dual"):
    val errors = typeCheckErrors(
      """
        val general = scalafim.multivar.optimization.TypedGeneralMap.instance[Int, Int](
          scalafim.multivar.optimization.TypedMapDescriptor(
            "general",
            scalafim.multivar.optimization.MapCapability.General,
            Vector.empty,
            scalafim.multivar.contract.FrameSymmetry.Identity
          ),
          identity
        )
        general.dual
