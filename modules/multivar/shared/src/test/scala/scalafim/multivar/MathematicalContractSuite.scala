...
class MathematicalContractSuite extends munit.FunSuite:
  ...
  test("reference roles cannot be interchanged at compile time"):
    val errors = typeCheckErrors("""
      val api = scalafim.multivar.ContractReference.api("api").toOption.get
      val formula = scalafim.multivar.ContractReference.formula("formula").toOption.get
      val ir = scalafim.multivar.ContractReference.ir("ir").toOption.get
      scalafim.multivar.FormulaBinding(api, formula, ir)
    """)
