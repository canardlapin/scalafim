...
class GridCompatibilitySuite extends munit.FunSuite:
  ...
  test("pointwise volume arithmetic rejects a different physical grid") {
    ...
    val left = NeuroVol.fromLinear(NArrayUtil.fillConst[Double](4, 1.0), expected)
    val right = NeuroVol.fromLinear(NArrayUtil.fillConst[Double](4, 2.0), reflected)
  ...
  test("volume concatenation rejects a different physical grid") {
    ...
    val translated = NeuroSpace(Vector(2, 2, 1), trans = Some(translatedAffine))
    val left = NeuroVol.fromLinear(NArrayUtil.fillConst[Double](4, 1.0), expected)
    val right = NeuroVol.fromLinear(NArrayUtil.fillConst[Double](4, 2.0), translated)
  ...
  test("mask-based sparse conversion rejects a different physical grid") {
    val expected = NeuroSpace(Vector(2, 2, 1))
    val translated = NeuroSpace(Vector(2, 2, 1), trans = Some(translatedAffine))
    val volume = NeuroVol.fromLinear(NArrayUtil.fillConst[Double](4, 1.0), expected)
    val mask = NeuroVol.fromLinear(NArray[Boolean](true, true, false, false), translated)
