...
class FunctionalImageSuite extends munit.FunSuite:
  ...
  private def vector(values: NArray[Int]): Vector[Int] =
    Vector.tabulate(values.length)(values.apply)

  test("mapValues and coordinate-aware volume mapping preserve geometry") {
    val volume = NeuroVol.fromLinear(NArray[Int](10, 20, 30, 40), volumeSpace.toNeuroSpace)
    ...
    assertEquals(vector(mapped.values.data), Vector(11, 21, 31, 41), clue = "")
    assertEquals(vector(located.values.data), Vector(10, 21, 40, 51), clue = "")
  }

  test("NeuroVec mapping distinguishes voxel coordinates from time samples") {
    val space = volumeSpace.addTime(2)
    val series = NeuroVec.fromLinear(NArray[Int](0, 0, 0, 0, 0, 0, 0, 0), space.toNeuroSpace)
    val mapped = series.mapSamples: (coord, time, _) =>
      coord.x + 10 * coord.y + 100 * time

    assertEquals(vector(mapped.values.data), Vector(0, 1, 10, 11, 100, 101, 110, 111), clue = "")
  }

  test("checked zipWith rejects a different physical grid") {
    val left = NeuroVol.fromLinear(NArray[Int](1, 2, 3, 4), volumeSpace.toNeuroSpace)
    val right = NeuroVol.fromLinear(NArray[Int](10, 20, 30, 40), volumeSpace.toNeuroSpace)
    val translated = NeuroVol.fromLinear(NArray[Int](10, 20, 30, 40), translatedSpace.toNeuroSpace)

    val summed = left.zipWith(right)(_ + _).fold(error => fail(error.message), identity)
    assertEquals(vector(summed.values.data), Vector(11, 22, 33, 44), clue = "")
    assert(left.zipWith(translated)(_ + _).isLeft)
  }

  test("effectful traversal sequences failures without adding an image monad") {
    val volume = NeuroVol.fromLinear(NArray[Int](1, 2, 3, 4), volumeSpace.toNeuroSpace)
    ...
    assertEquals(success.map(result => vector(result.values.data)), Some(Vector(2, 4, 6, 8)), clue = "")
  ...
  test("RoiSeries mapping retains selection and exposes coordinates") {
    val series = NeuroVec.fromLinear(
      NArray[Int](0, 1, 2, 3, 10, 11, 12, 13),
      volumeSpace.addTime(2).toNeuroSpace
    )
    val selection =
      VoxelSelection.make(volumeSpace, NArray[Int](2, 0))
  ...
  test("region algebra and selection compose through Either pipelines") {
    ...
    val left =
      VoxelRegion.make(volumeSpace, NArray[Int](0, 2))
        .fold(error => fail(error.message), identity)
    val right =
      VoxelRegion.make(volumeSpace, NArray[Int](1, 2))
        .fold(error => fail(error.message), identity)
  ...
  test("sparse selection requires an explicit missing-voxel policy") {
    ...
    val dense = NArray[Int](0, 1, 2, 3, 10, 11, 12, 13)
    val mask = Mask.fromIndices(volumeSpace.toNeuroSpace, NArray[Int](0, 2))
    val sparse = SparseNeuroVec.fromDense[Int](dense, seriesSpace.toNeuroSpace, mask)
    val selection =
      VoxelSelection.make(volumeSpace, NArray[Int](2, 1, 0))
        .fold(error => fail(error.message), identity)
    ...
    val translatedSelection =
      VoxelSelection.make(translatedSpace, NArray[Int](0))
        .fold(error => fail(error.message), identity)
