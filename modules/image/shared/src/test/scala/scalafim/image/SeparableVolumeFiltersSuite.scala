...
class SeparableVolumeFiltersSuite extends munit.FunSuite:
  test("separable box sums match the independent rectangular oracle"):
    ...
    val expected = NArrayUtil.ofSize[Double](grid.nVoxels)
  ...
  test("truncated box sums are self-adjoint"):
    ...
    val boxLeft = NArrayUtil.ofSize[Double](grid.nVoxels)
    val boxRight = NArrayUtil.ofSize[Double](grid.nVoxels)
    val workspace = BoxSumWorkspace(grid)
  ...
  test("physical Gaussian reflection preserves constants and anisotropic scale"):
    ...
    val smooth = NArrayUtil.ofSize[Double](grid.nVoxels)
  ...
  test("support-normalized Gaussian propagates a constant without mask pinning"):
    ...
    while z <= 6 do
      ...
      while y <= 7 do
        ...
        y += 1
      z += 1
    val destination = NArrayUtil.ofSize[Double](grid.nVoxels)
  ...
  test("native oblique pyramid sampling preserves a world-linear field and gradient"):
    ...
    val sourceValues = NArrayUtil.ofSize[Double](sourceGrid.nVoxels)
    ...
    val target = DenseFieldKernels.pyramidGrid(sourceGrid, 2)
    val sampled = NArrayUtil.ofSize[Double](target.nVoxels)
    val valid = NArrayUtil.ofSize[Boolean](target.nVoxels)
    ...
    val gradients = NArrayUtil.ofSize[Double](target.nVoxels * 3)
    val gradientValid = NArrayUtil.ofSize[Boolean](target.nVoxels)
  ...
  private def values(grid: GridSpec)(f: Int => Double): narr.NArray[Double] =
  ...
  private def fillWorldLinear(grid: GridSpec, destination: narr.NArray[Double]): Unit =
    var index = 0
    while index < grid.nVoxels do
  ...
  private def dot(left: narr.NArray[Double], right: narr.NArray[Double]): Double =
    var total = 0.0
    var index = 0
