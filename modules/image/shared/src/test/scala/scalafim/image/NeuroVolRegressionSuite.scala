...
class NeuroVolRegressionSuite extends munit.FunSuite:
  ...
  test("SparseNeuroVol.toDense preserves indices (others are zero)") {
    ...
    val idx = NArray[Int](1, 4, 9)
    val vals = NArray[Double](1.0, 2.0, 3.0)
    val sv = SparseNeuroVol[Double](vals, idx, sp)
    val dense = sv.toDense

    val out = Vector.tabulate(dense.values.data.length)(i => dense.values.data(i))
  ...
  test("NeuroVol.asLogical flags all non-zero values") {
    ...
    val vol = NeuroVol.fromLinear[Double](NArray[Double](0.0, -1.0, 2.0, 0.0), sp)
    val m = vol.asLogical
    val flags = Vector.tabulate(m.values.data.length)(i => m.values.data(i))
  ...
  test("NeuroVol.asMask(indices) sets specified indices true") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    val vol = NeuroVol.fromLinear[Double](NArrayUtil.fillConst[Double](4, 0.0), sp)
    val m = vol.asMask(NArray[Int](1, 3))
    val flags = Vector.tabulate(m.values.data.length)(i => m.values.data(i))
  ...
  test("NeuroVol.asSparse from mask/indices roundtrips when outside-mask values are zero") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    val vol = NeuroVol.fromLinear[Double](NArray[Double](0.0, 1.0, 0.0, 1.0), sp)
    val mask = vol.asMask
    ...
    val v0 = Vector.tabulate(vol.values.data.length)(i => vol.values.data(i))
    val v1 = Vector.tabulate(dense1.values.data.length)(i => dense1.values.data(i))
    val v2 = Vector.tabulate(dense2.values.data.length)(i => dense2.values.data(i))
  ...
  test("SparseNeuroVol.toMask matches stored indices") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    val idx = NArray[Int](1, 3)
    val vals = NArray[Double](10.0, 20.0)
    val sv = SparseNeuroVol[Double](vals, idx, sp)
    val mask = sv.toMask
    val back = Mask.indices(mask)
    val got = Vector.tabulate(back.length)(i => back(i))
  ...
  test("SparseNeuroVol rejects out-of-range indices") {
    ...
    intercept[IllegalArgumentException] {
      SparseNeuroVol[Double](NArray[Double](1.0), NArray[Int](8), sp)
    }
  }
  ...
  test("SparseNeuroVol rejects mismatched data/indices lengths") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    intercept[IllegalArgumentException] {
      SparseNeuroVol[Double](NArray[Double](1.0, 2.0), NArray[Int](0), sp)
    }
  }

  test("NeuroVol.asMatrix matches linear ordering") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    val vol = NeuroVol.fromLinear[Int](NArray[Int](1, 2, 3, 4), sp)
    val m = vol.asMatrix
    assertEquals(m.shape, Vector(4, 1), clue = "")
    val mv = Vector.tabulate(m.data.length)(i => m.data(i))
    assertEquals(mv, Vector(1, 2, 3, 4), clue = "")
  }

  test("mapValues maps label ids with numeric keys and defaults missing to 0") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    val vol = NeuroVol.fromLinear[Int](NArray[Int](1, 2, 1, 2), sp)

    val out = vol.mapValues(Map(1 -> 10, 2 -> 20), default = 0)
    val vals = Vector.tabulate(out.values.data.length)(i => out.values.data(i)).distinct.sorted
    assertEquals(vals, Vector(10, 20), clue = "")

    val out2 = vol.mapValues(Map(1 -> 10), default = 0)
    val vals2 = Vector.tabulate(out2.values.data.length)(i => out2.values.data(i)).distinct.sorted
  ...
  test("mapf produces expected size and respects mask") {
    ...
    val vol = NeuroVol.fromLinear[Double](NArrayUtil.fillConst[Double](nels, 1.0), sp)
    ...
    val sum = Vector.tabulate(out.values.data.length)(i => out.values.data(i)).sum
  ...
  test("NeuroVol(ROIVol) extracts values at ROI coords") {
    ...
    val nels = sp.spatialDims.product
    val vol = NeuroVol.fromLinear[Int](NArrayUtil.tabulate[Int](nels)(i => i + 1), sp)

    val coords = Vector(Vector(0, 0, 0), Vector(1, 0, 0), Vector(0, 1, 0), Vector(3, 3, 3))
    val roi = ROIVol[Int](sp, coords, NArrayUtil.fillConst[Int](coords.length, 0))
    val out = vol(roi)

    val got = Vector.tabulate(out.length)(i => out(i))
  ...
  test("NeuroVol.toVec produces a 4D NeuroVec with one volume") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    val vol = NeuroVol.fromLinear[Double](NArray[Double](1.0, 2.0, 3.0, 4.0), sp)
    val vec = vol.toVec
    assertEquals(vec.space.dims, Vector(2, 2, 1, 1), clue = "")
    val v0 = vec.volume(0)
    val got = Vector.tabulate(v0.values.data.length)(i => v0.values.data(i))
    assertEquals(got, Vector(1.0, 2.0, 3.0, 4.0), clue = "")
  }

  test("NeuroVol.concat stacks volumes along time into a NeuroVec") {
    val sp = NeuroSpace(Vector(2, 2, 1))
    val v1 = NeuroVol.fromLinear[Double](NArray[Double](1.0, 2.0, 3.0, 4.0), sp)
    val v2 = NeuroVol.fromLinear[Double](NArray[Double](10.0, 20.0, 30.0, 40.0), sp)
    val vec = v1.concat(v2)
    assertEquals(vec.space.dims, Vector(2, 2, 1, 2), clue = "")
    val lin = Vector.tabulate(vec.values.data.length)(i => vec.values.data(i))
  ...
  test("NeuroVol.slices returns per-plane NeuroSlice objects") {
    ...
    val data = NArrayUtil.ofSize[Double](nx * ny * nz)
    var z = 0
    while z < nz do
      var y = 0
      while y < ny do
