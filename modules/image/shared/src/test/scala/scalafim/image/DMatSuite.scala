...
class DMatSuite extends FunSuite:
  ...
  test("fromRowMajorOwned rejects invalid dimensions and buffer lengths") {
    intercept[IllegalArgumentException] {
      DMat.fromRowMajorOwned(0, 2, PrimitiveBuffers.ofSize[Double](0))
    }
    intercept[IllegalArgumentException] {
      DMat.fromRowMajorOwned(2, 2, PrimitiveBuffers.ofSize[Double](3))
    }
    intercept[IllegalArgumentException] {
      DMat.fromRowMajorOwned(Int.MaxValue, 2, PrimitiveBuffers.ofSize[Double](1))
    }
  }
