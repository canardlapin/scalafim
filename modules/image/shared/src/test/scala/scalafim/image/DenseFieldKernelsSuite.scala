...
class DenseFieldKernelsSuite extends munit.FunSuite:
    ...
    while z < grid.shape.z do
      ...
      while y < grid.shape.y do
        ...
        while x < grid.shape.x do
          ...
          values(i) = mapped.x
          values(i + grid.nVoxels) = mapped.y
          values(i + 2 * grid.nVoxels) = mapped.z
          x += 1
        y += 1
      z += 1
    DenseVectorField(
  ...
  private def allTrue(values: narr.NArray[Boolean]): Boolean =
    var result = true
    var i = 0
  ...
  test("identity and scalar pull preserve a physical world-linear volume") {
    ...
    val identity = DenseFieldKernels.identity(grid)
    val pulled = DenseFieldKernels.pullScalar(
      source,
      identity.field,
      FieldValidity.Mask(identity.valid)
    )
  ...
  test("channel pull shares interpolation support and uses SoA channel storage") {
    val grid = GridSpec.identity(Vector(4, 3, 2))
    val channels = 2
    val source = NArrayUtil.ofSize[Double](grid.nVoxels * channels)
    var i = 0
    while i < grid.nVoxels do
      source(i) = i.toDouble
      source(i + grid.nVoxels) = 100.0 - 2.0 * i.toDouble
      i += 1
    val identity = DenseFieldKernels.identity(grid)
    val pulled = DenseFieldKernels.pullChannels(
      NDArray(source, grid.dims :+ channels),
      grid,
      identity.field
    )
    ...
    i = 0
    while i < grid.nVoxels do
      assertClose(pulled.values.data(i), source(i))
      assertClose(pulled.values.data(i + grid.nVoxels), source(i + grid.nVoxels))
      i += 1
  }
  ...
  test("pull validity rejects masked, non-finite, and outside interpolation support") {
    ...
    val map = DenseFieldKernels.identity(grid)
    val mapValues = map.field.values.data
    mapValues(0) = 0.5
    mapValues(grid.nVoxels) = 0.5
    mapValues(2 * grid.nVoxels) = 0.5
    mapValues(1) = -0.25
    val sourceMask = NArrayUtil.fillConst[Boolean](grid.nVoxels, true)
    sourceMask(index(grid, 1, 1, 1)) = false
    val mapMask = NArrayUtil.fillConst[Boolean](grid.nVoxels, true)
    mapMask(2) = false
    val pulled = DenseFieldKernels.pullScalar(
      source,
      map.field,
      FieldValidity.Mask(mapMask),
      FieldValidity.Mask(sourceMask),
      outside = -99.0
    )
  ...
  test("affine-fused scalar pull matches an explicitly transformed coordinate field") {
    ...
    val expected = DenseFieldKernels.pullScalar(source, explicit, outside = -1.0)
    val actual = NArrayUtil.ofSize[Double](grid.nVoxels)
    val actualValid = NArrayUtil.ofSize[Boolean](grid.nVoxels)
  ...
  test("nearest scalar pull preserves labels on an oblique grid and propagates validity") {
    ...
    val map = DenseFieldKernels.identity(grid)
    val values = map.field.values.data
    ...
    values(first) = firstSource.x
    values(first + grid.nVoxels) = firstSource.y
    values(first + 2 * grid.nVoxels) = firstSource.z
    values(second) = secondSource.x
    values(second + grid.nVoxels) = secondSource.y
    values(second + 2 * grid.nVoxels) = secondSource.z
    val sourceValidity = NArrayUtil.fillConst[Boolean](grid.nVoxels, true)
    sourceValidity(index(grid, 3, 2, 1)) = false
    val mapValidity = NArrayUtil.fillConst[Boolean](grid.nVoxels, true)
    mapValidity(third) = false

    val pulled = DenseFieldKernels.pullScalarNearest(
      source,
      map.field,
      FieldValidity.Mask(mapValidity),
      FieldValidity.Mask(sourceValidity),
      outside = -1.0
    )
  ...
  test("dense pull composition matches analytic affine composition and existing morphism sampling") {
    ...
    val composed = DenseFieldKernels.composePull(left, right)
    val rightMorphism =
    ...
    while z < grid.shape.z - 1 do
      ...
      while y < grid.shape.y - 1 do
        ...
        while x < grid.shape.x - 2 do
          ...
          assertClose(composed.field.values.data(i), expected.x, 1e-10)
          assertClose(composed.field.values.data(i + grid.nVoxels), expected.y, 1e-10)
          assertClose(composed.field.values.data(i + 2 * grid.nVoxels), expected.z, 1e-10)
          x += 1
        y += 1
      z += 1
  }
  ...
  test("composition invalidity does not clamp and writes an identity fallback") {
    ...
    assert(!composed.valid(i))
    assertClose(composed.field.values.data(i), 0.0)
    assertClose(composed.field.values.data(i + grid.nVoxels), 1.0)
    assertClose(composed.field.values.data(i + 2 * grid.nVoxels), 1.0)
  }
  ...
  test("identity extension preserves an out-of-grid self-map query") {
    ...
    val left = field(grid)(p => WorldPoint(p.x - 0.25, p.y, p.z))
    val identity = DenseFieldKernels.identity(grid)
    val composed = DenseFieldKernels.composePull(
      left,
      identity.field,
      rightValidity = FieldValidity.Mask(identity.valid),
      outside = CoordinateMapOutside.Identity
    )
    val i = index(grid, 0, 1, 1)

    assert(composed.valid(i))
    assertClose(composed.field.values.data(i), -0.25)
    assertClose(composed.field.values.data(i + grid.nVoxels), 1.0)
    assertClose(composed.field.values.data(i + 2 * grid.nVoxels), 1.0)
  }
  ...
  test("linear regridding preserves an affine physical-coordinate field") {
    ...
    val regridded = DenseFieldKernels.regridPull(source, fine)
    val reference =
    ...
    while i < fine.nVoxels do
      ...
      assertClose(regridded.field.values.data(i), expected.x, 1e-10)
      assertClose(regridded.field.values.data(i + fine.nVoxels), expected.y, 1e-10)
      assertClose(regridded.field.values.data(i + 2 * fine.nVoxels), expected.z, 1e-10)
      assertClose(regridded.field.values.data(i), reference(i)(0), 1e-10)
      i += 1
  }
  ...
  test("identity-extended self-map regridding remains valid beyond sampled support") {
    ...
    val source = DenseFieldKernels.identity(sourceGrid)
    val strict = DenseFieldKernels.regridPull(source.field, targetGrid)
    val extended = DenseFieldKernels.regridPull(
      source.field,
      targetGrid,
      outside = CoordinateMapOutside.Identity
    )
    ...
    assertClose(extended.field.values.data(outsideIndex), 4.0)
    assertClose(extended.field.values.data(outsideIndex + targetGrid.nVoxels), 1.0)
    assertClose(extended.field.values.data(outsideIndex + 2 * targetGrid.nVoxels), 1.0)
  }
  ...
  test("Jacobian determinants are physical-coordinate correct on an oblique grid") {
    ...
    val summary = DenseFieldKernels.jacobianDeterminantsInto(map, determinants, valid)
  ...
  test("Jacobian reduction detects a folded field and validity erosion") {
    ...
    val fieldMask = NArrayUtil.fillConst[Boolean](grid.nVoxels, true)
    fieldMask(index(grid, 2, 2, 2)) = false
    val determinants = NArrayUtil.ofSize[Double](grid.nVoxels)
    val valid = NArrayUtil.ofSize[Boolean](grid.nVoxels)
    val summary = DenseFieldKernels.jacobianDeterminantsInto(
      folded,
      determinants,
      valid,
      FieldValidity.Mask(fieldMask)
    )
  ...
  test("paired inverse error is zero for opposite sub-voxel translations") {
    ...
    val summary = DenseFieldKernels.inversePairError(forward, backward)
  ...
  test("inverse error reports a known physical and voxel-space bias") {
    ...
    val summary = DenseFieldKernels.inverseErrorReduce(first, second)
  ...
  test("anti-aliased pyramid suppresses checkerboard energy") {
    ...
    val sourceValues = NArrayUtil.ofSize[Double](grid.nVoxels)
    var z = 0
    while z < grid.shape.z do
      var y = 0
      while y < grid.shape.y do
    ...
    val level = DenseFieldKernels.buildPyramidLevel(source, shrink = 2, sigmaMm = 1.0)
  ...
  test("mask-normalized pyramid preserves a constant without boundary dilution") {
    ...
    val mask = NArrayUtil.fillConst[Boolean](grid.nVoxels, false)
    var z = 4
    while z <= 8 do
      var y = 4
      while y <= 8 do
        ...
        y += 1
      z += 1
    val level = DenseFieldKernels.buildPyramidLevel(
      source,
      shrink = 2,
      sigmaMm = 1.0,
      sourceValidity = FieldValidity.Mask(mask),
      minimumWeight = 1e-4
    )

    var validCount = 0
    var i = 0
    while i < level.values.values.data.length do
      if level.valid.linear(i) then
        assertClose(level.values.linear(i), 7.0, 1e-10)
        validCount += 1
      i += 1
    assert(validCount > 0)
    assert(validCount < level.values.values.data.length, clue = "far-outside voxels must remain invalid")
  }

  test("pyramid grids preserve origin and scale every physical basis column") {
    val source = affineGrid(Vector(10, 9, 8))
    val target = DenseFieldKernels.pyramidGrid(source, shrink = 3)
  ...
  test("prepared samplers preserve pull, Jacobian, inverse, and pyramid results") {
    ...
    val pulled = NArrayUtil.ofSize[Double](grid.nVoxels)
    val pullValid = NArrayUtil.ofSize[Boolean](grid.nVoxels)
    ...
    val determinants = NArrayUtil.ofSize[Double](grid.nVoxels)
    val determinantValid = NArrayUtil.ofSize[Boolean](grid.nVoxels)
    val jacobian = DenseFieldKernels.jacobianDeterminantsInto(
      identity.field,
      determinants,
      determinantValid,
      sampler,
    ...
    val inverse = DenseFieldKernels.inverseErrorReduce(
      identity.field,
      identity.field,
      sampler,
      sampler,
    ...
    val target = DenseFieldKernels.pyramidGrid(grid, shrink = 2)
    val pyramid = NArrayUtil.ofSize[Double](target.nVoxels)
    val pyramidValid = NArrayUtil.ofSize[Boolean](target.nVoxels)
  ...
  test("reusable reductions match immutable summaries and overwrite empty support") {
    ...
    val none = NArrayUtil.ofSize[Boolean](grid.nVoxels)
    var i = 0
  ...
  test("prepared samplers reject a mismatched field grid") {
    ...
    val smallIdentity = DenseFieldKernels.identity(small)
    val largeIdentity = DenseFieldKernels.identity(large)
    intercept[IllegalArgumentException] {
      DenseFieldKernels.composePullInto(
        smallIdentity.field,
        smallIdentity.field,
        NArrayUtil.ofSize[Double](small.nVoxels * 3),
        NArrayUtil.ofSize[Boolean](small.nVoxels),
  ...
  test("destination-writing kernels validate buffer contracts") {
    ...
    intercept[IllegalArgumentException] {
      DenseFieldKernels.pullScalarInto(
        ...
        NArrayUtil.ofSize[Boolean](grid.nVoxels),
        sourceValidity = FieldValidity.Mask(NArrayUtil.ofSize[Boolean](1))
      )
    }
  }
