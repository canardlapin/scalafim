...
class TypedImageCoreSuite extends munit.FunSuite:
  ...
  private def toVector(indices: NArray[Int]): Vector[Int] =
  ...
  test("validated index sets canonicalize masks and reject duplicate sparse positions") {
    ...
    val indexSet = VoxelIndexSet(space, NArray(2, 0, 2))
    ...
    assert(VoxelIndexSet.makeUnique(space, NArray(1, 1)).isLeft, clue = "sparse index sets should reject duplicates")

    intercept[IllegalArgumentException] {
      SparseNeuroVol(NArray(10.0, 20.0), NArray(1, 1), space)
    }
  }
  ...
  test("VoxelRoi validates coordinates before ROI extraction") {
    ...
    val vol = NeuroVol.fromLinear[Int](NArray(10, 20, 30), space)

    val values = vol(roi)
    assertEquals(Vector.tabulate(values.length)(i => values(i)), Vector(10, 30), clue = "")
