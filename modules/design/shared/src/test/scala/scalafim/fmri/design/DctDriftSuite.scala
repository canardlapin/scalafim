...
class DctDriftSuite extends munit.FunSuite:
  ...
  private def count(samples: Int, tr: Double, period: Double): Int =
    DctDrift.componentCount(samples, Seconds(tr), cutoff(period)).fold(error => fail(error.message), identity)
  ...
  test("mixed scan counts and TRs have variable run-local column spans and stable identities"):
    ...
    assert(model.designColmap.filter(_.role == ColumnRole.Drift)
      .forall(_.basisName.contains("dct_ii_period_bits_4618441417868443648")))
    assert(schema.columns.take(6).forall(_.origin match
      case StructuralColumnOrigin.Drift(_, Some(component), RunScope.Run(_)) =>
        component.basisId == "dct_ii_period_bits_4618441417868443648"
      case _ => false
    ))
  ...
  test("censoring selects original scan positions and acquisition-time identity"):
    ...
    val sliced = model.designSchema.runwiseSlice(RunIndex.unsafeOneBased(2), selected)
  ...
  test("the documented typed plan builds and legacy names remain unchanged"):
    ...
    val model = BaselineModel.buildEither(BaselinePlan(frame,
      basis = BaselineBasis.Dct(period), intercept = Intercept.Runwise))
      .fold(error => fail(error.message), identity)
