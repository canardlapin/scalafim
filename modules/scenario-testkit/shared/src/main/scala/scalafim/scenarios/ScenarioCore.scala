package scalafim.scenarios

/** The terminal verdict vocabulary shared by cross-built scenario suites. */
enum ScenarioStatus:
  case Pass, PassWithCaveats, Fail

  def ciPass: Boolean =
    this == Pass

enum CaveatSeverity:
  case Note, Actionable, Blocking

enum CaveatKind:
  case PublicApiGap, AlgorithmDivergence, FixtureFreshness, PerformanceBudget, DiagnosticsGap, ErgonomicPain

final case class ScenarioCaveat(
    id: String,
    kind: CaveatKind,
    severity: CaveatSeverity,
    owner: String,
    followUp: Option[String],
    detail: String
):
  require(id.trim.nonEmpty, "caveat id must be non-empty")
  require(owner.trim.nonEmpty, "caveat owner must be non-empty")
  require(detail.trim.nonEmpty, "caveat detail must be non-empty")
  followUp.foreach(value => require(value.trim.nonEmpty, "caveat follow-up must be non-empty when supplied"))

  def blocksCi: Boolean =
    severity == CaveatSeverity.Blocking

  def render: String =
    val suffix = followUp.fold("")(value => s" followUp=$value")
    s"caveat=$id kind=$kind severity=$severity owner=$owner detail=$detail$suffix"

final case class ScenarioPolicy(
    allowedStatuses: Set[ScenarioStatus],
    allowedCaveatIds: Set[String]
):
  require(allowedStatuses.nonEmpty, "scenario policy must allow at least one status")
  require(!allowedStatuses.contains(ScenarioStatus.Fail), "scenario policy must not allow Fail")
  require(allowedCaveatIds.forall(_.trim.nonEmpty), "allowed caveat ids must be non-empty")

  def allows(result: ScenarioResult): Boolean =
    allowedStatuses.contains(result.status) &&
      result.caveats.forall(caveat => allowedCaveatIds.contains(caveat.id))

object ScenarioPolicy:
  val PassOnly: ScenarioPolicy =
    ScenarioPolicy(Set(ScenarioStatus.Pass), Set.empty)

  def allowCaveats(ids: String*): ScenarioPolicy =
    ScenarioPolicy(
      allowedStatuses = Set(ScenarioStatus.Pass, ScenarioStatus.PassWithCaveats),
      allowedCaveatIds = ids.toSet
    )

final case class ScenarioTolerance private (absolute: Double, relative: Double):
  require(absolute >= 0.0 && absolute.isFinite, "absolute tolerance must be finite and non-negative")
  require(relative >= 0.0 && relative.isFinite, "relative tolerance must be finite and non-negative")

  def threshold(expected: Double): Double =
    absolute + relative * math.max(1.0, math.abs(expected))

object ScenarioTolerance:
  def absolute(value: Double): ScenarioTolerance =
    ScenarioTolerance(value, 0.0)

  def mixed(absolute: Double, relative: Double): ScenarioTolerance =
    ScenarioTolerance(absolute, relative)

enum ScenarioMetricKind:
  case AbsoluteL2Error, RelativeL2Error, SignedCorrelation, NormRatio

final case class ScenarioComparisonTolerance private (
    maxAbsoluteL2Error: Double,
    maxRelativeL2Error: Double,
    minimumSignedCorrelation: Double,
    maxNormRatioDeviation: Double
):
  require(maxAbsoluteL2Error >= 0.0 && maxAbsoluteL2Error.isFinite, "absolute L2 limit must be finite and non-negative")
  require(maxRelativeL2Error >= 0.0 && maxRelativeL2Error.isFinite, "relative L2 limit must be finite and non-negative")
  require(
    minimumSignedCorrelation >= -1.0 && minimumSignedCorrelation <= 1.0 && minimumSignedCorrelation.isFinite,
    "minimum signed correlation must be finite and in [-1, 1]"
  )
  require(maxNormRatioDeviation >= 0.0 && maxNormRatioDeviation.isFinite, "norm-ratio limit must be finite and non-negative")

object ScenarioComparisonTolerance:
  def bounded(
      maxAbsoluteL2Error: Double,
      maxRelativeL2Error: Double,
      minimumSignedCorrelation: Double,
      maxNormRatioDeviation: Double
  ): ScenarioComparisonTolerance =
    ScenarioComparisonTolerance(
      maxAbsoluteL2Error,
      maxRelativeL2Error,
      minimumSignedCorrelation,
      maxNormRatioDeviation
    )

enum ScenarioObservation:
  case Scalar(name: String, actual: Double, expected: Double, tolerance: ScenarioTolerance)
  case Metric(
      name: String,
      kind: ScenarioMetricKind,
      actual: Double,
      expected: Double,
      tolerance: ScenarioTolerance
  )
  case Fact(name: String, ok: Boolean, detail: String)

  def passed: Boolean =
    this match
      case Scalar(_, actual, expected, tolerance) =>
        actual.isFinite && expected.isFinite && math.abs(actual - expected) <= tolerance.threshold(expected)
      case Metric(_, _, actual, expected, tolerance) =>
        actual.isFinite && expected.isFinite && math.abs(actual - expected) <= tolerance.threshold(expected)
      case Fact(_, ok, _) =>
        ok

  def render: String =
    this match
      case Scalar(name, actual, expected, tolerance) =>
        val delta =
          if actual.isFinite && expected.isFinite then math.abs(actual - expected) else Double.NaN
        s"$name: actual=$actual expected=$expected delta=$delta tol=${tolerance.threshold(expected)} pass=$passed"
      case Metric(name, kind, actual, expected, tolerance) =>
        val delta =
          if actual.isFinite && expected.isFinite then math.abs(actual - expected) else Double.NaN
        s"$name: metric=$kind actual=$actual expected=$expected delta=$delta tol=${tolerance.threshold(expected)} pass=$passed"
      case Fact(name, ok, detail) =>
        s"$name: $detail pass=$ok"

final case class ScenarioResult(
    id: String,
    observations: Vector[ScenarioObservation],
    caveats: Vector[ScenarioCaveat] = Vector.empty
):
  require(id.trim.nonEmpty, "scenario id must be non-empty")
  require(observations.nonEmpty, "scenario must contain at least one observation")
  require(caveats.map(_.id).distinct.length == caveats.length, "scenario caveat ids must be unique")

  def status: ScenarioStatus =
    if observations.exists(!_.passed) then ScenarioStatus.Fail
    else if caveats.exists(_.blocksCi) then ScenarioStatus.Fail
    else if caveats.nonEmpty then ScenarioStatus.PassWithCaveats
    else ScenarioStatus.Pass

  def ciPass: Boolean =
    ciPass(ScenarioPolicy.PassOnly)

  def ciPass(policy: ScenarioPolicy): Boolean =
    policy.allows(this)

  def failures: Vector[ScenarioObservation] =
    observations.filterNot(_.passed)

  def render: String =
    val lines =
      Vector(s"scenario=$id status=$status") ++
        observations.map(observation => s"  ${observation.render}") ++
        caveats.map(caveat => s"  ${caveat.render}")
    lines.mkString("\n")

/** A numeric view keeps matrix ownership in the module that owns the matrix type. */
trait ScenarioMatrix:
  def rows: Int
  def cols: Int
  def apply(row: Int, col: Int): Double

/** A numeric view keeps vector ownership in the module that owns the vector type. */
trait ScenarioVector:
  def length: Int
  def apply(index: Int): Double

object ScenarioHarness:
  def result(
      id: String,
      observations: Vector[ScenarioObservation],
      caveats: Vector[ScenarioCaveat] = Vector.empty
  ): ScenarioResult =
    ScenarioResult(id, observations, caveats)

  def scalar(
      name: String,
      actual: Double,
      expected: Double,
      tolerance: ScenarioTolerance
  ): ScenarioObservation =
    ScenarioObservation.Scalar(name, actual, expected, tolerance)

  def metric(
      name: String,
      kind: ScenarioMetricKind,
      actual: Double,
      expected: Double,
      tolerance: ScenarioTolerance
  ): ScenarioObservation =
    ScenarioObservation.Metric(name, kind, actual, expected, tolerance)

  def fact(name: String, passed: Boolean, detail: String): ScenarioObservation =
    ScenarioObservation.Fact(name, passed, detail)

  def finite(name: String, values: Iterable[Double]): ScenarioObservation =
    val xs = values.toVector
    val nonFinite = xs.zipWithIndex.collect {
      case (value, index) if !value.isFinite => s"$index=$value"
    }
    fact(
      name,
      nonFinite.isEmpty,
      if nonFinite.isEmpty then s"${xs.length} finite values"
      else nonFinite.mkString("non-finite values: ", ", ", "")
    )

  def values(
      name: String,
      actual: IndexedSeq[Double],
      expected: IndexedSeq[Double],
      tolerance: ScenarioTolerance
  ): Vector[ScenarioObservation] =
    Vector(
      fact(
        s"$name.length",
        actual.length == expected.length,
        s"actual=${actual.length} expected=${expected.length}"
      )
    ) ++
      actual.zip(expected).zipWithIndex.map {
        case ((a, e), index) => scalar(s"$name[$index]", a, e, tolerance)
      }.toVector

  /** Compare a numerical output through complementary scale and shape metrics.
    *
    * The profile always reports all four metrics. In particular, a scaled or
    * sign-reversed vector cannot pass merely because its correlation is high.
    */
  def comparisonMetrics(
      name: String,
      actual: IndexedSeq[Double],
      expected: IndexedSeq[Double],
      tolerance: ScenarioComparisonTolerance
  ): Vector[ScenarioObservation] =
    val domainValid =
      actual.nonEmpty &&
        actual.length == expected.length &&
        actual.forall(_.isFinite) &&
        expected.forall(_.isFinite)
    val domain = fact(
      s"$name.metrics.domain",
      domainValid,
      s"actual=${actual.length} expected=${expected.length} finite=${actual.forall(_.isFinite) && expected.forall(_.isFinite)}"
    )

    var actualSquared = 0.0
    var expectedSquared = 0.0
    var errorSquared = 0.0
    var actualMean = 0.0
    var expectedMean = 0.0
    if domainValid then
      var index = 0
      while index < actual.length do
        val a = actual(index)
        val e = expected(index)
        actualSquared += a * a
        expectedSquared += e * e
        val difference = a - e
        errorSquared += difference * difference
        actualMean += a
        expectedMean += e
        index += 1
      actualMean /= actual.length.toDouble
      expectedMean /= expected.length.toDouble

    var covariance = 0.0
    var actualCenteredSquared = 0.0
    var expectedCenteredSquared = 0.0
    if domainValid then
      var index = 0
      while index < actual.length do
        val actualCentered = actual(index) - actualMean
        val expectedCentered = expected(index) - expectedMean
        covariance += actualCentered * expectedCentered
        actualCenteredSquared += actualCentered * actualCentered
        expectedCenteredSquared += expectedCentered * expectedCentered
        index += 1

    val absoluteError = if domainValid then math.sqrt(errorSquared) else Double.NaN
    val expectedNorm = if domainValid then math.sqrt(expectedSquared) else Double.NaN
    val actualNorm = if domainValid then math.sqrt(actualSquared) else Double.NaN
    val relativeError =
      if domainValid && expectedNorm > 0.0 then absoluteError / expectedNorm else Double.NaN
    val correlationDenominator = math.sqrt(actualCenteredSquared * expectedCenteredSquared)
    val signedCorrelation =
      if domainValid && correlationDenominator > 0.0 then
        math.max(-1.0, math.min(1.0, covariance / correlationDenominator))
      else Double.NaN
    val normRatio =
      if domainValid && expectedNorm > 0.0 then actualNorm / expectedNorm else Double.NaN

    Vector(
      domain,
      metric(
        s"$name.absolute-l2-error",
        ScenarioMetricKind.AbsoluteL2Error,
        absoluteError,
        0.0,
        ScenarioTolerance.absolute(tolerance.maxAbsoluteL2Error)
      ),
      metric(
        s"$name.relative-l2-error",
        ScenarioMetricKind.RelativeL2Error,
        relativeError,
        0.0,
        ScenarioTolerance.absolute(tolerance.maxRelativeL2Error)
      ),
      metric(
        s"$name.signed-correlation",
        ScenarioMetricKind.SignedCorrelation,
        signedCorrelation,
        1.0,
        ScenarioTolerance.absolute(1.0 - tolerance.minimumSignedCorrelation)
      ),
      metric(
        s"$name.norm-ratio",
        ScenarioMetricKind.NormRatio,
        normRatio,
        1.0,
        ScenarioTolerance.absolute(tolerance.maxNormRatioDeviation)
      )
    )

  def vector(
      name: String,
      actual: ScenarioVector,
      expected: ScenarioVector,
      tolerance: ScenarioTolerance
  ): Vector[ScenarioObservation] =
    val length = fact(
      s"$name.length",
      actual.length == expected.length,
      s"actual=${actual.length} expected=${expected.length}"
    )
    Vector(length) ++
      (0 until math.min(actual.length, expected.length)).toVector.map { index =>
        scalar(s"$name[$index]", actual(index), expected(index), tolerance)
      }

  def matrix(
      name: String,
      actual: ScenarioMatrix,
      expected: ScenarioMatrix,
      tolerance: ScenarioTolerance
  ): Vector[ScenarioObservation] =
    val shape = Vector(
      fact(s"$name.rows", actual.rows == expected.rows, s"actual=${actual.rows} expected=${expected.rows}"),
      fact(s"$name.cols", actual.cols == expected.cols, s"actual=${actual.cols} expected=${expected.cols}")
    )
    val rows = math.min(actual.rows, expected.rows)
    val cols = math.min(actual.cols, expected.cols)
    val values =
      (0 until rows).toVector.flatMap { row =>
        (0 until cols).toVector.map { col =>
          scalar(s"$name[$row,$col]", actual(row, col), expected(row, col), tolerance)
        }
      }
    shape ++ values
