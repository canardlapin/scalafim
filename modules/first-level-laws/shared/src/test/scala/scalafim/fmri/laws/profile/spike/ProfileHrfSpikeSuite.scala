...
/** PHRF-25 spike: kernel-basis condition fit end to end, against a dense time-domain oracle, with strict work counters
  * and phase timings. This is an experiment, not a production estimator; see docs/plans/profile-hrf-work-items-v2.md.
  */
class ProfileHrfSpikeSuite extends munit.FunSuite:
  ...
  private lazy val fullBasis = SpikeKernelBasis.compile(
    domain,
    fineDt,
    horizon,
    tauNodes = 26,
  ...
  private lazy val schedule = SpikeSchedule.random(
    conditions,
    eventsPerCondition,
    nT * stride,
    nFine,
    nT,
    stride,
    new scala.util.Random(20260910L)
  )
  ...
  test("kernel basis rank report"):
    ...
    println(
      s"[spike]   chosen m=$basisRank for value tolerance $basisTolerance; K=${prep.rank} (C*m=${conditions * basisRank}), nuisance rank=${prep.nuisanceRank}"
    )
  ...
  test("compact energy and amplitudes agree with the direct time-domain fit at a prepared shape"):
    ...
    val cohort = SpikeCohort.generate(
      schedule,
      nuisance,
      nuisanceCols,
      basis.fineTimes,
      domain.tauMin + truthMargin,
      domain.tauMax - truthMargin,
      domain.logSdMin + 0.2,
      domain.logSdMax - 0.2,
      voxels = 8,
      snr = 1.0,
    ...
    val oracle =
      new SpikeOracle(schedule, prep, whitener, basis.fineTimes, domain, tauGrid = 3, logSdGrid = 3, compassLevels = 0)
    ...
    while v < cohort.voxels do
      ...
      while i < conditions do
        worstBeta = math.max(
          worstBeta,
          math.abs(solver.lastAmplitudes(i) - oracleOut.beta(i)) / math.max(1e-9, math.abs(oracleOut.beta(i)))
        )
        i += 1
      v += 1
    println(
      f"[spike] prepared-shape agreement: max |E_compact - E_direct|/e = $worstEnergy%.3e, max rel amplitude diff = $worstBeta%.3e"
    )
    assert(worstEnergy < 1e-5, s"energy discrepancy $worstEnergy")
    assert(worstBeta < 1e-3, s"amplitude discrepancy $worstBeta")

  private def oracleDirect(
      oracle: SpikeOracle,
      wy: Array[Double],
      tau: Double,
      logSd: Double,
      out: SpikeOracleResult
  ): Double =
  ...
  private final case class Variant(
      name: String,
      tauNodes: Int,
      logSdNodes: Int,
      steps: Int,
  ...
  test("decoded shapes agree with the dense oracle across SNR levels"):
    ...
    for snr <- Seq(1.0, 0.5, 0.25) do
      ...
      val oracleMs = now() - t0
      println(
        f"[spike] SNR $snr%.2f: oracle ${oracleMs / voxels}%.2f ms/voxel, ${oracleEvals.toDouble / voxels}%.1f exact time-domain evaluations/voxel"
      )
    ...
    assert(
      overall,
      s"accuracy gates unmet at SNR >= 0.5 for the default variant ${variants(defaultVariant).name}; see [spike] output"
    )
    ...
    for tol <- Seq(1e-3, 1e-4) do
      ...
      for snr <- Seq(1.0, 0.5) do
        val cohort = SpikeCohort.generate(
          ...
          snr,
          arPhi,
          new scala.util.Random(101L + (if snr == 1.0 then 0 else 1))
        )
        val wy = whitener(voxels, transpose(cohort.y, voxels, nT))
        ...
        while v < voxels do
          ...
          v += 1
        println(
          f"[spike] basis tol $tol m=$rank K=${pr.rank} SNR $snr%.2f ${vn.name}: admitted $admitted/$voxels, p95 |tau-or| ${percentile(tauErr.result(), 0.95)}%.4f s, p95 rel amp ${percentile(ampErr.result(), 0.95)}%.2e, ${solverMs / voxels}%.3f ms/voxel"
        )
  ...
  test("benchmark: V=10,000 condition fit with phase timings"):
    ...
    println(
      f"[spike] benchmark V=$voxels T=$nT C=$conditions K=${prep.rank} m=${basis.m}: generate ${gen}%.0f ms, whiten (ar) ${wh}%.0f ms, project ${pr}%.0f ms (per-voxel loop) / ${pr4}%.0f ms (4-wide blocked)"
    )
    variants.foreach { vn =>
      ...
      assert(
        counters.perVoxel(counters.jets) <= vn.steps + 1e-9,
        s"${vn.name}: jets ${counters.perVoxel(counters.jets)}"
      )
