...
class ObservedFamilyCertificationSuite extends munit.FunSuite:
  ...
  private val onsets =
    Vector(1.3, 6.1, 10.6, 17.0, 23.4, 30.7, 36.2, 41.9, 47.5, 53.8, 60.3, 66.1, 71.0, 77.4).map(Seconds(_))
  ...
  private val nuisance = DMat.tabulate(rows, 3)((t, j) =>
    if j == 0 then 1.0 else if j == 1 then t.toDouble / rows else math.cos(math.Pi * (t + 0.5) / rows)
  )
  private val heldOut =
    Vector((3.4, math.log(0.9)), (4.8, math.log(1.6)), (6.1, math.log(2.4)), (7.7, math.log(1.1)), (5.5, math.log(2.9)))
  ...
  test("the observed family is certified under whitening and nuisance projection"):
    ...
    assert(
      certificate.maxDesignError <= 5.0 * basis.certificate.valueError(basis.rank - 1),
      s"design error ${certificate.maxDesignError}"
    )
  ...
  test("an aliased condition cell is refused as rank loss, not certified"):
    ...
    val aliasedExpanded =
      ExpandedConditionDesign.lower(aliased, frame, basis, precision).fold(e => fail(e.message), identity)
    ObservedFamilyCertification.certify(
      aliasedExpanded,
      aliased,
      frame,
      precision,
