...
object LnaExplicitLatent:
  val MetadataKindKey: String = "lna.response.kind"
  val MetadataKindValue: String = "explicit_latent"
  ...
  def isExplicitEmbed(desc: TransformDescriptor): Boolean =
    desc.kind == TransformKind.Embed &&
      (desc.datasets.exists(_.role == DatasetRole.Loadings) || explicitMetadataMarker(desc))
  ...
  private def explicitMetadataMarker(desc: TransformDescriptor): Boolean =
    desc.params match
      case p: TransformParams.Embed =>
        p.metadata.get(MetadataKindKey).contains(MetadataKindValue)
      case _ =>
        false
