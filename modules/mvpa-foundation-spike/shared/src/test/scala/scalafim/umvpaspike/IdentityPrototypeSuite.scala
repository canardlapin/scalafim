...
""")
    ...
    assertEquals(wrongColumn.length, 1)
    assert(wrongColumn.head.message.contains("Required: scalafim.umvpaspike.IdentityPrototype.Restriction[S]"))
    assertEquals(wrongTarget.length, 1)
    assert(wrongTarget.head.message.contains("Required: scalafim.umvpaspike.IdentityPrototype.Restriction[S]"))
    assertEquals(wrongFrame.length, 1)
    assert(wrongFrame.head.message.contains("Required: scalafim.umvpaspike.IdentityPrototype.Measurement[N]"))
