...
class BasisExpandedRetentionSuite extends munit.FunSuite:
  ...
  private lazy val model: FmriModel =
    ...
    val data = ImageDMat.fromRows(Vector.tabulate(n)(t => Vector.tabulate(voxels)(voxel => responses(voxel)(t))))
    val dataset = FmriDataset.unsafe(
      InMemoryDatasetBackend(DatasetId("basis-expanded"), data, NeuroSpace(Vector(voxels, 1, 1))), frame)
