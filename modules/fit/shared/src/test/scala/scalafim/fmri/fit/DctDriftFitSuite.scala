...
class DctDriftFitSuite extends munit.FunSuite:
  ...
  test("native selected betas and contrasts equal independent FWL on complete and censored original rows"):
    ...
    val events = DatasetEvents(Vector(0, 5, 11, 16, 24, 31).zipWithIndex.map: (onset, i) =>
    ...
    val dataset = FmriDataset.unsafe(
      InMemoryDatasetBackend(DatasetId("dct-selected-fwl"), GaleTestMatrix.fromRows(values), SampleSpaces(Vector(2, 1, 1))),
      frame, events)
    ...
    val rowSets = Vector((0 until 48).toVector,
      (0 until 48).filterNot(i => Set(0, 2, 7, 12, 23, 36, 47)(i)).toVector.reverse)
    for rows <- rowSets do
      ...
      val outcome = right(prepared.foreachBlock(reader, block =>
        ...
        block.voxelIndices.zipWithIndex.foreach: (voxel, local) =>
