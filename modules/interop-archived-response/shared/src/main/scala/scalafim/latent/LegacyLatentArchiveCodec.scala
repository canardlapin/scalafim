...
object LegacyLatentArchiveCodec:
  def toArchive(
...
/** Compatibility alias for the pre-registry central archive dispatch.
  *
  * New representation families install an `ArchivedResponseFamily` in an
  * application registry and must not add branches here.
  */
@deprecated(
  "Use explicit archived-response families; compatibility code may use LegacyLatentArchiveCodec",
  "0.2.0"
)
object LatentArchiveCodec:
  export LegacyLatentArchiveCodec.*
