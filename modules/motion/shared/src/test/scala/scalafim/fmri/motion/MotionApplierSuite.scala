...
class MotionApplierSuite extends munit.FunSuite:
  ...
  private def assertSameValues(actual: NeuroVec[Double], expected: Vector[Double], tol: Double = 1e-12): Unit =
    val actualValues = actual.copyLegacyLinear
  ...
  test("identity motion application is one-pass and preserves data") {
    ...
    assertSameValues(corrected, run.copyLegacyLinear.toVector)
  }
  ...
  test("all-zero slice timing is byte-identical to volume application") {
    ...
    assertSameValues(timedCorrected, volumeCorrected.copyLegacyLinear.toVector)
  }
  ...
  test("nonzero slice timing interpolates packet poses during final application") {
    ...
    assertEqualsDouble(volumeCorrected.linear(3), 10.0, 1e-12)
    assertEqualsDouble(timedCorrected.linear(3), 10.0, 1e-12)
    assertEqualsDouble(volumeCorrected.linear(4), 11.0, 1e-12)
    assertEqualsDouble(timedCorrected.linear(4), 10.0, 1e-12)
    assertEqualsDouble(volumeCorrected.linear(5), 12.0, 1e-12)
    assertEqualsDouble(timedCorrected.linear(5), 11.0, 1e-12)
  }
