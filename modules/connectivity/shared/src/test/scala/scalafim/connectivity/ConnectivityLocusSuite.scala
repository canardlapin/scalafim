...
class ConnectivityLocusSuite extends munit.FunSuite:
  ...
  test("node finite spaces preserve VertexBasis order and semantic identity"):
    ...
    assertEquals(
      locus.space.indices.map(locus.nodeAt).map(_.id.value).toVector,
      Vector("a", "b", "c")
    )
    assert(
